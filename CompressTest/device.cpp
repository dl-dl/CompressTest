...
void Device::Run()
{
 while (GpsReady(deviceId))
  {
   PointFloat d;
   GetGps(&d, deviceId);
   ProcessGps(d);
  }
 while (RadioReady(deviceId))
  {
   RadioMsg d;
   GetRadio(&d, deviceId);
   ProcessRadio(&d);
  }
 while (CompassReady(deviceId))
  {
   CompassData d;
   GetCompass(&d, deviceId);
   ProcessCompass(d);
  }
 while (ButtonReady(deviceId))
  {
   ui8 d;
   GetButton(&d, deviceId);
   ProcessButton(d);
  }
 if (timer)
  {
